...
def check_process_post_scan (_id, _mode, local_data):  
    ...
    print(_id, " : ", _mode," : WIP callback check_process_post_scan")
    #Will be use later
    fs = local_data.get("fs")
    #Get only usefull column scan id and tsv status
    result = local_data.get("dataframe")[['scan_id','STATUS']].drop_duplicates()
    
    #Replace by process OK or associated error code
    result.STATUS=result.STATUS.map(lambda x: labels.errors["process_post_scan.unprocessed"] if labels.errors["global.missing_ecotaxa_table"]==x
                                              else labels.sucess["process_post_scan.ok"])
    
    #Rename collums to match the desiered output
    result.rename(columns={'scan_id' : 'List scan ID', 'STATUS' : 'Process POST SCAN'}, inplace=True)

    #TODO attendre la reponse d'amanda pour completer ce test
    return result
