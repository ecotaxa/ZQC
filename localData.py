...
def missingCol(cols, path):
    ...
    if type(path) is dict:
        ...
        read_cols = pd.read_csv(zip_file.open(
            path["tsv"]), nrows=0, encoding="utf-8", sep="\t").columns
    else:
        read_cols = pd.read_csv(
            path, nrows=0, encoding="ISO-8859-1", sep="\t").columns
...
def getdata(mode, subpath):
    ...
    if mode == Mode.TSV:
        ...
        # Get checked_files data
        checked_files = getCheckedFiles(subpath)

        return {"dataframe": dataframe, "fs": fsData, "meta": meta_files, "checked_files": checked_files}
...
def getTsv(subpath):
    ...
    try:
        for folder_name in listFolder(base_path+subpath+"/Zooscan_scan/_work/"):
            try:
                path = base_path+subpath+"/Zooscan_scan/_work/" + \
                    folder_name+"/ecotaxa_"+folder_name+".tsv"
                ...
                df = pd.DataFrame(data={'scan_id': [
                                  folder_name], 'STATUS': labels.errors["global.missing_ecotaxa_table"]})
                df[cols] = labels.errors["global.missing_ecotaxa_table"]
                tsv_files.append(df)
                logging.warning("{}".format(e))
    except IOError as e:
        df = pd.DataFrame(data={'scan_id': [
                          "NOSCANID"], 'STATUS': labels.errors["global.missing_directory.work"]})
    ...
    try:
        ...
        try:
            ...
            try:
                for file in os.listdir(folder_name):
                    if file.endswith(".zip"):
                        zips.append({"zip": os.path.join(
                            folder_name, file), "tsv": "ecotaxa_"+file.replace('.zip', '.tsv')})
            except:
                raise ValueError(
                    labels.errors["multiples.missing_directory.ecotaxa"])
            ...
            df = pd.read_csv(zip_file.open(
                zip_path["tsv"]), encoding="utf-8", usecols=cols_ok, sep="\t")
            df['STATUS'] = ""
...
def getHeader(subpath):
    ...
    try:
        df = pd.read_csv(base_path+subpath+"/Zooscan_meta/zooscan_sample_header_table.csv",
                         encoding="ISO-8859-1", usecols=['sampleid', 'ship'], sep=";")
    ...
    try:
        df = pd.read_csv(
            base_path+subpath+"/Zooscan_meta/zooscan_scan_header_table.csv", encoding="ISO-8859-1", sep=";")
        header_files.append(df)
    except IOError as e:
        logging.warning("{}".format(e))
    return header_files
...
def getCheckedFiles(subpath):
    """Read another usefull file : /Zooscan_check/checked_files.txt for the given project. Return it as pandas dataframes"""
    checked_files = []
    #  read file and add an entry in meta_files dictionary
    path = base_path+subpath+"/Zooscan_check/checked_files.txt"
    try:
        with open(path) as f:
            lines = f.readlines()
            checked_files= lines
...
def _recursive_folderstats(folderpath, items=None,
    ...
    if os.access(folderpath, os.R_OK):
        dirs = list(filter(lambda x: "DS_Store" not in x,
        ...
        for f in dirs:
            ...
            if os.path.isdir(filepath):
                ...
                try:
                    inside_name = ZipFile(filepath, 'r').namelist(
                    ) if extension == "zip" else "None"
