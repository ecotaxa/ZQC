...
def update_projects_dropdown(value):
    ...
    return [
        {'label': project['label'] + " 🔒" if project['disabled'] else project['label'], 'value': project['label'],
         'disabled': project['disabled']} for project in projects
    ]
...
@app.callback([Output('tabs-content-before_scan', 'children'), Output("runQC-btn-before_scan", 'n_clicks'),
               Output('tabs-before_scan', 'value')],
              [Input("tabs-before_scan", 'value'), Input("runQC-btn-before_scan", 'n_clicks'),
               Input('app-1-dropdown-projects', "value")],
...
def render_content_before_scan(tab, click_run, projects, drive):
    if not click_run:
        if tab == 'tab-details-before_scan':
            ...
            return componants.generate_result(
                "This feature will be available in a future release of the QC application."), 0, tab
        else:
            return [], 0, tab
    # else:
    #     if len(projects) > 0:
    ...
    return componants.generate_result(
        "This feature will be available in a future release of the QC application."), 0, 'tab-result-before_scan'


# during_analysis Tabs related callbacks ##
@app.callback([Output('tabs-content-during_analysis', 'children'), Output("runQC-btn-during_analysis", 'n_clicks'),
               Output("tabs-during_analysis", 'value'), Output('intermediate-value-during_analysis', 'data'),
               Output("open-during_analysis", 'hidden')],
              [Input("tabs-during_analysis", 'value'), Input("runQC-btn-during_analysis", 'n_clicks'),
               Input('app-1-dropdown-projects', "value")],
              [State('app-1-dropdown-drives', 'value')], prevent_initial_call=True)
def render_content_during_analysis(tab, click_run, projects, drive):
    ...
    return componants.generate_result(
        componants.emptyResult("during_analysis", projects)), 0, 'tab-result-during_analysis', None, True


## after_ecotaxa_classif Tabs related callbacks ##
@app.callback(
    [Output('tabs-content-after_ecotaxa_classif', 'children'), Output("runQC-btn-after_ecotaxa_classif", 'n_clicks'),
     Output("tabs-after_ecotaxa_classif", 'value'), Output('intermediate-value-after_ecotaxa_classif', 'data'),
     Output("open-after_ecotaxa_classif", 'hidden')],
    [Input("tabs-after_ecotaxa_classif", 'value'), Input("runQC-btn-after_ecotaxa_classif", 'n_clicks'),
     Input('app-1-dropdown-projects', "value")],
    [State('app-1-dropdown-drives', 'value')], prevent_initial_call=True)
def render_content_after_ecotaxa_classif(tab, click_run, projects, drive):
    if not click_run:
        if tab == 'tab-details-after_ecotaxa_classif':
            ...
            return componants.generate_result(
                componants.emptyResult("after_ecotaxa_classif", projects)), 0, tab, None, True
        else:
            return [], 0, tab, None, True
    else:
        if len(projects) > 0:
    ...
    return componants.generate_result(
        componants.emptyResult("after_ecotaxa_classif", projects)), 0, 'tab-result-after_ecotaxa_classif', None, True


@app.callback(Output("notification-container", "sendNotifications"), Output("saveQC-btn", 'n_clicks'),
              Output("open-after_ecotaxa_classif", "n_clicks"), Output("open-during_analysis", "n_clicks"),
              Output("close", "n_clicks"), Output("modal-save", "is_open"),
...
def save_report(n_clicks_open_after_ecotaxa_classif, n_clicks_open_during_analysis, n_clicks_close, n_clicks_save,
                jsonified_pdf_data_after_ecotaxa_classif, jsonified_pdf_data_during_analysis, operator_first_name,
                operator_email, operator_last_name, operator_first_name_pattern, operator_email_pattern,
                operator_last_name_pattern, is_open):
    ...
    # if everything is ok for save : save
        ...
        if operator_first_name and operator_email and operator_last_name_pattern and re.search(
            operator_first_name_pattern, operator_first_name) and re.search(operator_email_pattern,
                                                                            operator_email) and re.search(
            operator_last_name_pattern, operator_last_name):
            ...
            for i in range(0, len(pdf_data)):
                pdf_data[i]["operator"] = {"name": operator_first_name.title(), "last_name": operator_last_name.upper(),
